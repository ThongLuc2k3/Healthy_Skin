...
router.post('/agent',requireAuth,async(req,res,next)=>{try{const user=await getUser(req.auth.sub);res.json({data:await planAgent(req.body.message,req.body.history,{user:{displayName:user.display_name,areaLabel:user.area_label,defaultUniversityId:user.default_university_id},now:new Date().toISOString()})})}catch(error){next(error)}})
