...
router.get('/', requireAuth, (req, res) => {
  res.json(listMilestones(req.userId))
})
...
router.delete('/:id', requireAuth, (req, res) => {
  ...
  const deleted = deleteMilestone(req.userId, id)
  if (!deleted) {
...
router.get('/:id/compare', requireAuth, (req, res) => {
  const result = compareMilestone(req.userId, Number(req.params.id))
  if (!result) {
    return res.status(404).json({ error: 'Không tìm thấy milestone.' })
  }
  res.json(result)
