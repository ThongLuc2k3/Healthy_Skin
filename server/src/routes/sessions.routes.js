...
import { checkIn,completeSession,listMySessions,openRequestDispute,reportNoShow,reviewSession,updateAppointmentDetails } from '../services/sessionService.js'
...
router.put('/:id/appointment',async(req,res,next)=>{try{res.json({data:await updateAppointmentDetails(req.auth.sub,req.params.id,req.body)})}catch(error){next(error)}})
