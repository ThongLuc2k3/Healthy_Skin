...
export async function updateAppointmentDetails(userId,requestId,input={}){
  const exactLocation=String(input.exactLocation||'').trim(),meetingUrl=String(input.meetingUrl||'').trim();if(exactLocation.length>500||meetingUrl.length>1000||meetingUrl&&!/^https:\/\//i.test(meetingUrl))throw Object.assign(new Error('Địa điểm hoặc liên kết họp không hợp lệ.'),{status:422});const db=database();if(!db){const request=demoRequests.find(x=>x.id===requestId&&x.author_id===userId);if(!request)throw Object.assign(new Error('Chỉ người đăng được cập nhật điểm hẹn.'),{status:403});request.exact_location=exactLocation||null;request.meeting_url=meetingUrl||null;return request}const {rows}=await db.query(`update appointments a set exact_location=$3,meeting_url=$4 from requests r where a.request_id=r.id and r.id=$1 and r.author_id=$2 returning a.*`,[requestId,userId,exactLocation||null,meetingUrl||null]);if(!rows[0])throw Object.assign(new Error('Chỉ người đăng được cập nhật điểm hẹn.'),{status:403});return rows[0]
}
