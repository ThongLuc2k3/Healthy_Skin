...
export async function demoWithdraw(userId,amountVnd){if(!Number.isInteger(amountVnd)||amountVnd<10000||amountVnd>1000000)throw Object.assign(new Error('Số tiền rút phải từ 10.000đ đến 1.000.000đ.'),{status:422,code:'INVALID_DEMO_WITHDRAW'});const db=database();if(!db){const wallet=walletFor(userId);if(wallet.available_vnd<amountVnd)throw Object.assign(new Error('Số dư khả dụng không đủ.'),{status:402,code:'INSUFFICIENT_BALANCE'});wallet.available_vnd-=amountVnd;entry(userId,null,'debit',amountVnd,'demo_withdraw');return wallet}const client=await db.connect();try{await client.query('begin');await client.query('insert into wallets(user_id) values($1) on conflict do nothing',[userId]);const wallet=(await client.query('select * from wallets where user_id=$1 for update',[userId])).rows[0];if(Number(wallet.available_vnd)<amountVnd)throw Object.assign(new Error('Số dư khả dụng không đủ.'),{status:402,code:'INSUFFICIENT_BALANCE'});await client.query('update wallets set available_vnd=available_vnd-$2,updated_at=now() where user_id=$1',[userId,amountVnd]);await client.query(`insert into ledger_entries(user_id,direction,amount_vnd,entry_type) values($1,'debit',$2,'demo_withdraw')`,[userId,amountVnd]);await client.query('commit');return (await client.query('select * from wallets where user_id=$1',[userId])).rows[0]}catch(e){await client.query('rollback');throw e}finally{client.release()}}
