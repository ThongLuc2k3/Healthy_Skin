...
export default function AiChatbot() {
  ...
  return <div className="fixed bottom-5 right-4 z-[75] sm:bottom-7 sm:right-7">
    {open && <section className="mb-3 flex h-[min(620px,calc(100vh-110px))] w-[min(390px,calc(100vw-28px))] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
      <header className="flex items-center gap-3 bg-[#183153] px-5 py-4 text-white"><span className="relative flex h-11 w-11 items-center justify-center rounded-2xl bg-[#2D5BFF]"><MessageCircleQuestion size={24}/><Sparkles className="absolute -right-1 -top-1 text-[#B7F34A]" size={14}/><i className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-[#183153] bg-[#B7F34A]"/></span><div className="flex-1"><h2 className="font-black">Trợ lý AI TLUCS</h2><p className="text-xs text-blue-200">Hỗ trợ và tiếp nhận khiếu nại</p></div><button onClick={() => setOpen(false)} aria-label="Thu nhỏ chatbot"><ChevronDown/></button></header>
