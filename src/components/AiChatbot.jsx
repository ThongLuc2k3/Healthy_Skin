...
export default function AiChatbot() {
  ...
  return <div className="fixed bottom-5 right-4 z-[75] sm:bottom-7 sm:right-7">
    {open && <section className="mb-3 flex h-[min(620px,calc(100vh-110px))] w-[min(390px,calc(100vw-28px))] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-4">{messages.map((message, index) => <div key={index} className={`max-w-[88%] whitespace-pre-line rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === 'user' ? 'ml-auto rounded-br-md bg-[#2D5BFF] text-white' : 'rounded-bl-md border bg-white text-slate-600 shadow-sm'}`}>{message.text}{message.role==='bot'&&(message.toolsUsed?.length>0||message.mode==='rag'||message.provider)&&<p className="mt-2 border-t border-slate-100 pt-2 text-[10px] font-bold text-slate-400">{message.mode==='rag'?(message.toolsUsed?.length?'RAG ≥ 85%':'Không có tài liệu RAG khớp ≥ 85%'):`Agent ${message.provider==='groq'?'Groq':message.provider==='gemini'?'Gemini dự phòng':'nội bộ'} · ${message.steps||1} bước${message.toolsUsed?.length?` · ${message.toolsUsed.join(', ')}`:''}`}</p>}</div>)}{busy && <div className="w-fit rounded-2xl bg-white px-4 py-3 text-sm text-slate-400 shadow-sm">{awaitingComplaint ? 'Đang gửi đến bộ phận hỗ trợ...' : 'Agent đang suy luận và gọi tool...'}</div>}<div ref={endRef}/></div>
